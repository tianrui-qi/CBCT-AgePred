...
class Trainer:
    ...
    def fit(self, evalu: bool = 0) -> None:
        self._load_ckpt()
        # evalu
        if evalu: 
            self._evalu_epoch()
    ...
    def _valid_epoch(self) -> None:
        ...
        for i, (frames, labels) in tqdm.tqdm(
    ...
    def _evalu_epoch(self) -> None:
        ...
        for i, (frames, labels) in tqdm.tqdm(
            enumerate(self.evaluloader),
            total=len(self.evaluloader),
            desc="evalu_epoch", leave=False, unit="steps", smoothing=1.0
        ):  
            # put frames and labels in GPU
            frames = frames.to(self.device)
            labels = labels.to(self.device)
            ...
            # record
            evalu_loss.append(loss_value.item())
